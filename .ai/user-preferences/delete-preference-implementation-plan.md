...
```typescript
const { data, error } = await supabase.from("user_preferences").delete().eq("id", id).eq("user_id", user.id).select();
 ...
 */
export async function deleteUserPreference(command: DeletePreferenceCommand): Promise<void> {
...
export const DELETE: APIRoute = async ({ params, locals, request }) => {
  try {
    ...
    if (!preferenceId || !isValidUUID(preferenceId)) {
      return createErrorResponse("INVALID_UUID", "Invalid preference ID format", 400);
    }
    ...
    if (error instanceof PreferenceNotFoundError) {
      return createErrorResponse("PREFERENCE_NOT_FOUND", "Preference not found", 404);
    }
