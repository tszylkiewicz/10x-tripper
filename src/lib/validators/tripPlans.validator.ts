 ...
 */
export const acceptTripPlanSchema = z.object({
  generation_id: z
    .string()
    .regex(UUID_V4_REGEX, "Invalid UUID format for generation_id")
    .optional()
    .nullable(),
 ...
 */
const tripPlanNotesSchema = z.object({
 ...
 */
export const generateTripPlanSchema = z.object({
  destination: z
    .string({
      required_error: "Destination is required",
      invalid_type_error: "Destination must be a string",
...
export function createGeneratePlanCommand(
  dto: GenerateTripPlanRequestDto,
  userId: string
): GeneratePlanCommand {
  return {
    ...dto,
